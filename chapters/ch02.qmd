...
For the HD-EEG source-reconstructed data, the same procedure was applied. However, the 22 frequency intervals proposed by @frauscher2018AtlasNormalIntracranial did not satisfy the 4% power condition for the reconstructed source spectra. However we decided the to use the same  frequency bins for the purpose for comparability across datasets.<!-- atisfying this condition were therefore defined separately for HD-EEG dataset -->. This should be note as a methodological caveat.

### Oscillatory peak detection: after 1/f removal


As the data itself has different charactering we used different setting for removing 
Spectral parameterisation: specparam

For the purpose of removing the 1/f background and testing if it improve the comparability we used specparam algorithm. 
