...
Both invasive and non-invasive recordings document systematic regional variation in spectral features. In intracranial electrocorticography, @groppe2013DominantFrequenciesResting identified region-specific spectral clusters, with theta dominant in frontal and temporal areas, alpha concentrated in parietal and occipital cortex, and beta prominent peri-Rolandically. In source-reconstructed MEG, @mahjoory2020FrequencyGradientHuman showed that dominant oscillatory frequency decreases along the posterior-to-anterior axis, tracking cortical hierarchies defined by cortical thickness and myelin content. @niso2019BrainstormPipelineAnalysis also demonstrated that resting-state MEG power can be mapped in source space using a reproducible pipeline. However, regional spectral organisation is not reducible to a single dominant frequency or a single anatomical gradient. @mellem2017IntrinsicFrequencyBiases found that many regions show intrinsic biases toward multiple frequency bands simultaneously, suggesting that regional spectral identity is distributed across frequency bands.
...
Comparing these features across recording modalities is methodologically difficult. iEEG provides a relatively direct measure of local field potentials with limited volume conduction, making it a useful reference for regional spectral organisation. Its signal quality advantage is substantial: @ball2009SignalQualitySimultaneously reported that the signal-to-noise ratio of invasive EEG is 20 to more than 100 times better than simultaneously obtained scalp EEG. However, iEEG is clinically constrained, spatially sparse, and biased by implantation coverage and patient populations. HD-EEG, by contrast, is non-invasive and can provide whole-head coverage, but source reconstruction introduces spatial blurring and method-specific biases that affect the estimated spectral profiles [@liu2018DetectingLargeScaleBrain; @lai2018ComparisonScalpSourcereconstructed; @piastra2020ComprehensiveStudyElectroencephalography]. @tait2021SystematicEvaluationSource showed that source reconstruction can introduce systematic distortions that propagate into spectral parameterisation.
...
This chapter addresses three related questions. irst, which aspects of regional spectral organisation reported in intracranial recordings are also visible in source-reconstructed HD-EEG? Second, do oscillatory and aperiodic features show comparable spatial organisation, or do they reveal different aspects of cortical spectral structure? Third, how do different ways of quantifying oscillatory activity relate to one another once broadband 1/f structure is modelled?
...
The HD-EEG dataset (n = 19) was harmonized with the MNI Open iEEG Atlas to allow for direct cross-modal comparison. High-density EEG data were obtained from the dataset described in @samogin2019SharedConnectionspecificIntrinsic and  @samogin2020FrequencydependentFunctionalConnectivity.
While the HD-EEG data were recorded with eyes open, the processing pipeline was strictly matched to the iEEG atlas: source activity was extracted from  using average coordinates from the  electrodes within 76 MICCAI-defined ROIs in MNI space,  resulting in 1444 virtual sensors, with the grouped regional reference coordinates illustrated in Figure @fig-frauscher-atlas, panel B.
